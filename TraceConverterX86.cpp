...
static inline int getFullRegister(int id) {
	if (id < 16)
		...
		return (32 + id - 16); //between al to bl
	else if (id < 24)
		return (32 + id - 20); //between ah to bh
	else
...
void TraceConverterX86::printInstruction(ostream& outs, const TRInstructionX86& insn, bool bOps, bool bVerbose) {
	...
	if (get_instruction_string(&(insn.insn), FORMAT_ATT, insn.eh.address, line, LINE_SIZE) != 0) {
		outs << line << " tid:" << insn.eh.tid << " op:" << unsigned(insn.eh.num_operands) << " pop:" << unsigned(insn.eh.num_poperands) << hex << "\t\n";
	} else {
		outs << "null\t\n";
	}
...
//  }
	...
	if (bOps) {
		...
		//list<OperandVal> opList;
		for (int i = 0; (insn.eh.operand[i].type != TNone) && (i < MAX_NUM_OPERANDS); i++) {
			//opList.push_front(); //this has the effect of doing the reversing
			outs << "\t";
			printOperand(insn.eh.operand[i], outs);
			outs << "\t\n";
		}
		outs << "\n";
		for (int i = 0; (insn.eh.poperand[i].type != TNone) && (i < MAX_NUM_OPERANDS); i++) {
			//opList.push_front(); //this has the effect of doing the reversing
			outs << "\t";
			printOperand(insn.eh.poperand[i], outs);
			outs << "\t\n";
		}
		//now we need to sort
//    opList.sort(operand_compare);
