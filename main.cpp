...
public:
	...
	int PrintBap(TraceReaderBinX86 &tr) {
...
public:
	...
	int PrintSummary(const TRInstructionX86 &insn) {
...
int process_arg(int argc, char **argv) {
	...
	opts.add_options()("help,h", "help message")("input,i", value<string>(), "input filename")("bpt,b", value<string>(), "output bpt name")
	// ("output,o", value<string>(), "output filename")
	("verbose,v", "print verbose messages")("bap", "TODO: modify this")("sum", "print summary");
	variables_map vm;
	...
	if (vm.count("bpt")) {
		g_sBptName = vm["bpt"].as<string>();
	}
	g_bToBpt = (0 != vm.count("bpt"));
	// if(vm.count("output"))
	// {
