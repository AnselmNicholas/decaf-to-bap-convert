...
int TraceReaderBinX86::readNextInstruction() {
	...
	ifs.read((char*) (&(insn.eh.num_poperands)), sizeof(insn.eh.num_poperands));
	...
	//now read in all the pre operands
...
// commented it out since we are not testing anymore
	...
	count = 0;
	// read post operand
	for (i = 0, j = 0; i < MAX_NUM_OPERANDS && count < insn.eh.num_poperands; i++) {
		if (readOperand(insn.eh.poperand[i])) {
			return (-1);
		}
		count++;
/*
		 //skip all memory register operations for post list as its identical to pre list
		 //if its not memory related then don't need to populate the memory registers
		 if(insn.eh.operand[i].type != TMemLoc && insn.eh.operand[i].type != TMemAddress)
		 {
		 insn.eh.memregs[i][0].type = TNone;
